...
// time measurement
	...
	int gettimeofday(struct timeval *tv, struct timezone *tz) {
		FILETIME ft; unsigned __int64 tmpres = 0; static int tzflag;
		if (NULL != tv) { GetSystemTimeAsFileTime(&ft);
			tmpres |= ft.dwHighDateTime; tmpres <<= 32;
			tmpres |= ft.dwLowDateTime; tmpres -= 11644473600000000; tmpres /= 10;
			tv->tv_sec = (long)(tmpres / 1000000UL);
			tv->tv_usec = (long)(tmpres % 1000000UL);}
		if (NULL != tz) { if (!tzflag) {_tzset(); tzflag++;}
			tz->tz_minuteswest = _timezone / 60; tz->tz_dsttime = _daylight;}
		return 0;}
#else
	#include <sys/time.h>
#endif
...
struct Vec {double x, y, z;
	...
	inline Vec onb(const Vec &n) const {
		Vec u, w, v = n;
		if (n.z < -0.9999999) {u = Vec( 0.0, -1.0, 0.0); w = Vec(-1.0, 0.0, 0.0);}
		else {const double a = 1.0/(1.0 + n.z); const double b = -n.x * n.y * a;
				u = Vec(1.0 - n.x * n.x * a, b, -n.x); w = Vec(b, 1.0 - n.y * n.y * a, -n.y);}
...
struct TKahanAdder {double sum, carry, y;
	TKahanAdder (const double b = 0.0) {sum = b; carry = 0.0; y = 0.0;}
	inline void add(const double b) {y = b - carry; const double t = sum + y; carry = (t - sum) - y; sum = t;}
};

// pinhole camera
struct TCamera {Vec o, u, v, w; double dist;
	void set(const Vec o_, const Vec l_, const double fov_) {
		o = o_; dist = PixelHeight / (2.0 * tan( (fov_ / 2.0) * (PI / 180.0) ));
		w = (l_ - o_).norm(); u = (w % Vec(0.0, 1.0, 0.0)).norm(); v = u % w;
	}
} Camera;
#define camera_id -2
...
// path data
struct Vert {Vec p, n; int id; Vert(){}; Vert(Vec p_, Vec n_, int id_) : p(p_), n(n_), id(id_) {}};
struct Path {Vert x[MaxEvents]; int n; Path(){n=0;}};
struct Contribution {double x, y; Vec c; Contribution(){}; Contribution(double x_, double y_, Vec c_) : x(x_), y(y_), c(c_) {}};
struct PathContribution {Contribution c[MaxEvents*MaxEvents]; int n; double sc; PathContribution(){n = 0; sc = 0.0;}};
void AccumulatePathContribution(const PathContribution pc, const double mScaling) {
	if (pc.sc == 0) return;
	for (int i = 0; i < pc.n; i++) {
		const int ix = int(pc.c[i].x), iy = int(pc.c[i].y); const Vec c = pc.c[i].c * mScaling;
...
struct TMarkovChain { double u[NumStates]; PathContribution C;
	TMarkovChain() {for (int i = 0; i < NumStates; i++) u[i] = rnd();}
...
void InitRandomNumbersByChain(const TMarkovChain MC) {for (int i = 0; i < NumStates; i++) prnds[i] = MC.u[i];}
void InitRandomNumbers() {for (int i = 0; i < NumStates; i++) prnds[i] = rnd();}


// local sampling PDFs and standard terms
inline double GeometryTerm(const Vert e0, const Vert e1) {
	const Vec dv = e1.p - e0.p; const double d2 = dv.dot(dv);
	return fabs(e0.n.dot(dv) * e1.n.dot(dv)) / (d2 * d2);
}
inline double DirectionToArea(const Vert current, const Vert next) {
	const Vec dv = next.p - current.p; const double d2 = dv.dot(dv);
...
bool isConnectable(const Path Xeye, const Path Xlight, double &px, double &py) {
	...
	if ( (Xeye.n == 0) && (Xlight.n >= 2) ) {
		...
		double t; int id; Result = (intersect(r, t, id)) && (id == Xlight_e.id);
		Direction = r.d;
	} else {
...
void TracePath(Path &path, const Ray r, const int RayLevel, const int MaxRayLevel) {
	...
	double t; int id; if (!intersect(r,t,id)) return;
	const Sphere &obj = sph[id]; Vec p=r.o+r.d*t, n=(p-obj.p).norm();
	n = n.dot(r.d)<0 ? n : n*-1;

	// set path data
	path.x[path.n] = Vert(p, n, id); path.n++;
...
Path GenerateLightPath(const int MaxLightEvents)
{
	...
	Result.x[0] = Vert(r.o, n, light_id); Result.n++;
...
Path GenerateEyePath(const int MaxEyeEvents)
{
	...
	Result.x[0] = Vert(r.o, Camera.w, camera_id); Result.n++;
